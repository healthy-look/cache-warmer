...
async function fetchIndexSitemaps(domain, country) {
  try {
    ...
    return (Array.isArray(sitemapList) ? sitemapList : [sitemapList])
      .map((e) => e.loc)
      .filter(Boolean);
  } catch {
    return [];
...
async function fetchUrlsFromSitemap(sitemapUrl, country) {
  try {
    ...
    if (!urlList) return [];
    return (Array.isArray(urlList) ? urlList : [urlList])
      .map((e) => e.loc)
      .filter(Boolean);
  } catch {
    return [];
    ...
    await Promise.all(
      batch.map(async (url) => {
        ...
        try {
          ...
          // ✅ SATU-SATUNYA PERUBAHAN DI SELURUH FILE
          logger.log({
            country: cfEdge, // ← hanya ini
            url,
            status: res.status,
            cfCache,
            lsCache,
